...
struct SpaceModel3D{T, S<:Union{Int, Float64}, P<:SType} <:AbstractSpaceModel3D{T,S,P}
    ...
    SpaceModel3D{S,P}() where {S,P} =  begin #needed for initially attaching with agents
        ...
        patch_locs = [(1,1,1)]
        agents = Vector{Agent3D{S, P, MortalType}}()
        agents_added =  Vector{Agent3D{S, P, MortalType}}()
...
function Base.setproperty!(agent::Agent3D{S, P, MortalType}, key::Symbol, x) where {S<:Union{Int, Float64}, P<:SType}
    ...
    if (key!=:pos)
        ...
        update_grid!(agent, getfield(agent, :model)::SpaceModel3D{MortalType,S,P}, x)
    end
end

function Base.setproperty!(agent::Agent3D{S, P, StaticType}, key::Symbol, x) where {S<:Union{Int, Float64}, P<:SType}
    ...
    if (key!=:pos)
        ...
        update_grid!(agent, getfield(agent, :model)::SpaceModel3D{StaticType,S,P}, x)
    end
end
...
function update_grid!(agent::Agent3D{S, P, T}, model::SpaceModel3D{T,S,P}, pos) where {T<:MType,S<:Float64,P<:PeriodicType}
    x,y,z = pos
    i = getfield(agent, :id)
    size = model.size
...
function update_grid!(agent::Agent3D{S, P, T}, model::SpaceModel3D{T,S,P}, pos) where {T<:MType,S<:Float64,P<:NPeriodicType}
    x,y,z = pos
    i = getfield(agent, :id)
    size = model.size
    patches = model.patches
...
function update_grid!(agent::Agent3D{Int, P, T}, model::SpaceModel3D{T,S,P}, pos) where {T<:MType,S<:Int,P<:PeriodicType}
    x,y,z = pos
    x0,y0,z0 = agent.pos
    i = getfield(agent, :id)
    size = model.size
...
function update_grid!(agent::Agent3D{Int, P, T}, model::SpaceModel3D{T,S,P}, pos) where {T<:MType,S<:Int,P<:NPeriodicType}
    x,y,z = pos
    x0,y0,z0 = agent.pos
    i = getfield(agent, :id)
    size = model.size
