...
its own properties.  
- `periodic`` : If `periodic` is true the grid space will be periodic in both x and y directions. If set to false, agents can not be assigned position outside the 
space boundaries
