...
@testset "graph model" begin
    ...
    agents = create_graph_agents(5, color=:red, is_sick = false, shape = :circle, keeps_record_of = [:color, :is_sick])
    ...
    function initialiser(model)
        ...
        for vert in get_nodes(model.graph)
    ...
    function step_rule!(model)
        ...
        if model.tick == 4
            remove_edge!(2,3, model)
        end
        if model.tick ==5
            ags = get_agents(model)
            n = length(ags)
            remove_agent!(ags[rand(1:n)], model)
        end
        if model.tick ==7
            add_node!(7, model)
        end
    ...
    run_model!(model, steps=steps, step_rule= model_step!, mprops = [:model_property1])
